  ...
  <section class="bg-cream">
    <div class="section-container py-16 md:py-20">
      <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
        ...
        >
          <div
            class="mx-auto mb-4 h-14 w-14 rounded-2xl bg-primary/[0.06] flex items-center justify-center text-primary transition-all duration-300 group-hover:bg-primary group-hover:text-cream group-hover:shadow-soft-md group-hover:scale-105"
            aria-hidden="true"
            v-html="item.icon"
          ></div>
          <h3 class="text-sm font-semibold text-slate-900 mb-1">{{ $t(item.titleKey) }}</h3>
          <p class="text-xs text-slate-500 leading-relaxed">{{ $t(item.bodyKey) }}</p>
        </div>
      </div>
    </div>
  </section>
